...
func TestForMIA(t *testing.T) {
	t.Run("parsing of mia segment", func(t *testing.T) {
		...
		read, err = seg.Parse(in)
		require.Error(t, err)
		require.Equal(t, "mia segment has not enough input data", err.Error())
		require.Equal(t, 0, read)

		in = "DMT~"
		read, err = seg.Parse(in)
		require.Error(t, err)
		require.Equal(t, "mia segment contains invalid code", err.Error())
		require.Equal(t, 0, read)
	})
	...
	t.Run("parsing and encoding of mia segment with specified rule", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, len(in), read)
		require.Equal(t, "MIA*0019*00~", seg.String())

		seg.SetFieldByIndex("01", "0018")
		err = seg.Validate(nil)
		require.Error(t, err)
		require.Equal(t, "mia's element (01) has invalid value, the element contains unexpected value", err.Error())

		in = "MIA*0018*00~"
		read, err = seg.Parse(in)
		require.Error(t, err)
		require.Equal(t, "unable to parse mia's element (01), the element contains unexpected value", err.Error())
		require.Equal(t, 0, read)
	})
}
