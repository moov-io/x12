...
func TestForCreateSegment(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, seg)
	require.Equal(t, "CR6", seg.Name())

	seg, err = CreateSegment("CRC", nil)
	require.NoError(t, err)
	require.NotNil(t, seg)
	require.Equal(t, "CRC", seg.Name())
	...
	require.NoError(t, err)
	require.NotNil(t, seg)
	require.Equal(t, "QTY", seg.Name())

	seg, err = CreateSegment("RDM", nil)
	require.NoError(t, err)
	require.NotNil(t, seg)
	require.Equal(t, "RDM", seg.Name())
