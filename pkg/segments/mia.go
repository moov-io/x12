...
func (r MIA) defaultMask(index int) string {
	if index > 1 {
...
func (r MIA) fieldCount() int {
	return 24
}

func (r MIA) Name() string {
	return "MIA"
}
...
func (r *MIA) Validate(rule *rules.ElementSetRule) error {
	...
	for i := 1; i <= r.fieldCount(); i++ {
		...
		if err := util.ValidateField(r.GetFieldByIndex(idx), rule.Get(idx), r.defaultMask(i)); err != nil {
			return fmt.Errorf("mia's element (%s) has invalid value, %s", idx, err.Error())
		}
	}
...
func (r *MIA) Parse(data string, args ...string) (int, error) {
	...
	if length < int64(read) {
		return 0, errors.New("mia segment has not enough input data")
	} else {
		line = data[:length]
	}

	if r.Name() != data[:codeLen] {
		return 0, errors.New("mia segment contains invalid code")
	}
	...
	for i := 1; i <= r.fieldCount(); i++ {
		...
		if value, size, err = util.ReadField(line, read, r.GetRule().Get(idx), r.defaultMask(i), args...); err != nil {
			return 0, fmt.Errorf("unable to parse mia's element (%s), %s", idx, err.Error())
		} else {
			read += size
