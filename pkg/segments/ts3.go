...
func (r *TS3) Parse(data string, args ...string) (int, error) {
	...
	if length < int64(read) {
		return 0, errors.New("ts3 segment has not enough input data")
	} else {
		line = data[:length]
	}

	if r.Name() != data[:codeLen] {
		return 0, errors.New("ts3 segment contains invalid code")
	}
	...
	for i := 1; i <= r.fieldCount(); i++ {
		...
		if value, size, err = util.ReadField(line, read, r.GetRule().Get(idx), r.defaultMask(i), args...); err != nil {
			return 0, fmt.Errorf("unable to parse ts3's element (%s), %s", idx, err.Error())
		} else {
			read += size
