...
func (r PricingComposite) defaultMask() string {
	return rules.MASK_OPTIONAL
}

func (r PricingComposite) fieldCount() int {
	return 6
}

func (r *PricingComposite) SetFieldByIndex(index string, data any) error {
	return util.SetFieldByIndex(r, index, data)
}

func (r PricingComposite) GetFieldByIndex(index string) any {
	return util.GetFieldByIndex(r, index)
}

func (r *PricingComposite) Validate(rule *rules.ElementSetRule) error {
	if rule == nil {
		rule = r.GetRule()
	}
	...
	for i := 1; i <= r.fieldCount(); i++ {
		...
		if err := util.ValidateField(r.GetFieldByIndex(idx), rule.Get(idx), r.defaultMask()); err != nil {
			return fmt.Errorf("pricing composite's element (%s) has invalid value, %s", idx, err.Error())
		}
	}
...
func (r *PricingComposite) Parse(data string, args ...string) (int, error) {
	var err error
	var size, read int
	line := data
	...
	for i := 1; i <= r.fieldCount(); i++ {
		...
		if value, size, err = util.ReadCompositeField(line, read, r.GetRule().Get(idx), r.defaultMask(), args...); err != nil {
			return 0, fmt.Errorf("unable to parse pricing composite's element (%s), %s", idx, err.Error())
		} else {
			read += size
...
func (r PricingComposite) String(args ...string) string {
	var buf string
