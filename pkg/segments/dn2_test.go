...
func TestForDN2(t *testing.T) {
	...
	t.Run("parsing of dn2 segment", func(t *testing.T) {
		...
		read, err = seg.Parse(in)
		require.Error(t, err)
		require.Equal(t, "dn2 segment has not enough input data", err.Error())
		require.Equal(t, 0, read)

		in = "DMT~"
		read, err = seg.Parse(in)
		require.Error(t, err)
		require.Equal(t, "dn2 segment contains invalid code", err.Error())
		require.Equal(t, 0, read)
	})
	...
	t.Run("parsing and encoding of dn2 segment with specified rule", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, len(in), read)
		require.Equal(t, "DN2*0019*00~", seg.String())

		seg.SetFieldByIndex("01", "0018")
		err = seg.Validate(nil)
		require.Error(t, err)
		require.Equal(t, "dn2's element (01) has invalid value, the element contains unexpected value", err.Error())

		in = "DN2*0018*00~"
		read, err = seg.Parse(in)
		require.Error(t, err)
		require.Equal(t, "unable to parse dn2's element (01), the element contains unexpected value", err.Error())
		require.Equal(t, 0, read)
	})
}
